...
// ── Wizard Steps ──────────────────────────────────────────────────────
const currentStep = ref<string | number>('basic-info')

const stepItems: StepperItem[] = [
  { slot: 'basic-info' as const, value: 'basic-info', title: 'Basic Info', description: 'Module core details', icon: 'i-lucide-info' },
  { slot: 'entities' as const, value: 'entities', title: 'Entities', description: 'Select entities to include', icon: 'i-lucide-database' },
  { slot: 'navigation' as const, value: 'navigation', title: 'Navigation', description: 'Sidebar configuration', icon: 'i-lucide-compass' },
  { slot: 'permissions' as const, value: 'permissions', title: 'Permissions', description: 'Access control preset', icon: 'i-lucide-shield' },
  { slot: 'review' as const, value: 'review', title: 'Review', description: 'Confirm and create', icon: 'i-lucide-check-circle-2' }
]
      ...
      <UDashboardNavbar title="Create Module">
        ...
        <template #right>
          ...
          <UButton
            ...
            trailing
            :disabled="currentStep === 'basic-info' && !step1Valid"
            @click="() => { stepper?.next() }"
          />
        </template>
      </UDashboardNavbar>
    ...
    <template #body>
      <UContainer class="py-8">
        ...
        <UStepper
          ...
          :ui="{ content: 'p-6' }"
        >
          <!-- Per-step content via item.slot -->
          <template #basic-info>
