...
Standard Markdown elements auto-resolved by Comark/Content/MDC. Themed via `appConfig.ui.prose.<name>`.
...
Nuxt UI-specific Prose components. In markdown files they are used **without the `Prose` prefix** (e.g. `::callout`, `::steps`). Nuxt UI registers shorthand aliases for `Callout`: `::note`, `::tip`, `::warning`, `::caution`.
